...
class DrugBank:
    ...
    def get_matches(self, match_type: Literal["id", "name"]):
        ...
        for _, element in tqdm(ET.iterparse(self.filename, ["end"])):
            if element.tag[24:None:None] != "drug" or (element.attrib == {}):
                continue
...
if __name__ == "__main__":
    ...
    bbb = os.path.join("data", "admet", "bbb_martins-0.916-0.002.dump")
    pgp = os.path.join("data", "admet", "pgp_broccatelli-0.938-0.0.dump")
    hia = os.path.join("data", "admet", "hia_hou-0.989-0.001.dump")
    dili = os.path.join("data", "admet", "dili-0.918-0.005.dump")
    drugbank = os.path.join("data", "src", "drugbank.xml")

    # augment just the translator drugs first
    translator_drug_list = os.path.join("data", "translator_drugs.json")
    if os.path.exists(translator_drug_list) and os.path.getsize(translator_drug_list) > 0:
        augmenter = (
            DataAugmenter(translator_drug_list, "result_id", "id_type", "result_name")
            .load_drug_queries()
            .load_admet_models(
                {
                    "Blood Brain Barrier": bbb,
                    "P-glycoprotein Inhibition": pgp,
                    "Human Intestinal Absorption": hia,
                    "Drug Induced Liver Injury": dili,
                }
            )
        )
        augmenter.match_drugbank(drugbank)
        augmenter.make_main_name_col()
        augmenter.deduplicate()
        augmenter.predict_admet()
        translator_out_file = os.path.join("data", "augmented_translator_drugs.json")
        augmenter.save_drug_info(translator_out_file)

    # augment extra drugs
    extra_drug_list = os.path.join("data", "src", "extra_drugs.csv")
    if os.path.exists(extra_drug_list) and os.path.getsize(extra_drug_list) > 0:
        augmenter = (
            DataAugmenter(extra_drug_list, "result_id", "id_type", "result_name")
            .load_drug_queries()
            .load_admet_models(
                {
                    "Blood Brain Barrier": bbb,
        ...
        augmenter.deduplicate()
        augmenter.predict_admet()
        extra_out_file = os.path.join("data", "augmented_extra_drugs.json")
        augmenter.save_drug_info(extra_out_file)
